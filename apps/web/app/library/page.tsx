...
const documents:DocumentEntry[]=[
  ...
  {title:"Protocol Architecture",description:"从身份、贡献与证明，走向声誉、权利与协调。",kind:"Protocol",href:"/protocol",image:"/mood-world-hero.png",keywords:["architecture","proof","reputation","rights"]},
...
export default function LibraryPage(){
  ...
  return <main className="mood-library">
    ...
    <section className="library-browser" aria-labelledby="documents-title">
      ...
      <div className="document-grid">{visible.map((document,index)=><Link className="document-card" key={document.href} href={document.href}><img src={document.image} alt=""/><div className="document-card-copy"><div className="document-index">{String(index+1).padStart(2,"0")} · {document.kind}</div><h3>{document.title}</h3><p>{document.description}</p><span>Explore →</span></div></Link>)}{visible.length===0&&<p className="library-empty">没有找到匹配的内容。</p>}</div>
    </section>
